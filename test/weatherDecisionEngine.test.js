...
// 1
test('mild day produces a calm no-action brief',()=>{const result=buildWeatherDecisionEngine({weather:weather(),garden:garden([plant('kale','kale','bed','Established',{lastWatered:iso(-2)})]),now:NOW});assert.equal(result.brief[0].actionKey,'calm-weather')});

// 2
test('recent observed rain covers exposed bed without pretending protected spaces were watered',()=>{const w=weather({recentRain24h:.56});const exposed=wateringDecisionForPlant({plant:plant('tomato','tomato','bed'),space:spaces.bed,weather:w,now:NOW});const protectedDecision=wateringDecisionForPlant({plant:plant('pepper','bell-pepper','covered'),space:spaces.covered,weather:w,now:NOW});assert.equal(exposed.actionKey,'check-soil');assert.equal(protectedDecision.actionKey,'check-soil');assert.match(protectedDecision.explanation,/protected/i)});
...
test('indoor seedlings use tray moisture rather than outdoor rain credit',()=>{const p=plant('lettuce-tray','lettuce','inside','Seedling');const decision=wateringDecisionForPlant({plant:p,space:spaces.inside,weather:weather({recentRain24h:1}),now:NOW});assert.equal(decision.actionKey,'check-soil');assert.match(decision.explanation,/Indoor seedlings|outdoor rain/i)});

test('dry potato grow bag gets container-specific watering guidance',()=>{const potatoSpace={id:'bag',name:'Potato Bag',type:'potato-grow-bag'},p=plant('potato','potato','bag','Established',{moisture:'dry'}),decision=wateringDecisionForPlant({plant:p,space:potatoSpace,weather:weather({recentRain24h:0}),now:NOW});assert.equal(decision.actionKey,'water-now');assert.match(decision.explanation,/Potato grow bags dry faster/i)});
