...
test('no owned packet still creates useful discovery guidance',()=>{const result=buildGrowNowRecommendations({garden:baseGarden,recommendations:[],weather:calmWeather});const arugula=result.cards.find(row=>row.cropId==='arugula');assert.ok(arugula);assert.equal(arugula.owned,false);assert.ok(arugula.traits.length);assert.ok(arugula.actionNow);assert.ok(arugula.harvestTime)});
