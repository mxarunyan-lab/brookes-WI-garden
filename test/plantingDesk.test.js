...
test('weight-only seed packets stay plantable without fake seed quantity',()=>{const garden={...baseGarden,seedPackets:[{id:'weight',cropId:'lettuce',name:'Lettuce',variety:'Iceberg A',quantity:0,countType:'weight-only',packetWeight:'1.5 g'}]};const inventory=seedInventory(garden);assert.equal(inventory.length,1);assert.equal(inventory[0].quantityAvailable,null);assert.equal(inventory[0].uncountedQuantity,true)});
