...
test('all four seasonal paths use build-generated static WebP files',()=>{
 ...
 for(const[season,path]of Object.entries(SEASONAL_GARDEN_HEADERS))assert.match(path,new RegExp(`garden-header-${season}\\.webp\\?v=0478$`));
});
...
test('seasonal header stays focused on Today and preserves navigation',async()=>{
 const[main,runtime,css,today,navigation]=await Promise.all([
  read('src/main.jsx'),read('src/seasonalHeaderRuntime.js'),read('src/styles/phase-4-7-6-seasonal-header.css'),read('src/WorkspaceScreens.jsx'),read('src/Navigation.jsx')
 ]);
...
test('production build generates static WebP files from approved AVIF masters',async()=>{
 const[builder,pkg,server]=await Promise.all([read('scripts/buildSeasonalHeaders.mjs'),read('package.json'),read('server/index.js')]);
 assert.match(builder,/garden-header-\$\{season\}\.avif/);
 assert.match(builder,/garden-header-\$\{season\}\.webp/);
 assert.match(builder,/failOn:'none'/);
 assert.match(builder,/webp\(\{quality:95,effort:6,smartSubsample:true\}\)/);
 assert.match(pkg,/node scripts\/buildSeasonalHeaders\.mjs && vite build/);
 assert.doesNotMatch(server,/garden-header-:season\.webp/);
 assert.doesNotMatch(server,/seasonalHeaderCache/);
});
