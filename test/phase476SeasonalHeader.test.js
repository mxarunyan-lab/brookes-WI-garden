...
test('all four seasonal paths use versioned static WebP assets',()=>{
 ...
 for(const[season,path]of Object.entries(SEASONAL_GARDEN_HEADERS)){
  assert.equal(path,`/images/garden-headers/garden-header-${season}.webp?v=0479`);
 }
});
...
test('seasonal header stays focused on Today and preserves navigation',async()=>{
 ...
 assert.match(css,/compact-home-hero \.compact-hero-title/);
 assert.match(css,/background:#fffaf0!important/);
...
test('production build creates static lossless WebP headers before Vite',async()=>{
 const[packageJson,builder,server]=await Promise.all([
  read('package.json'),
  read('scripts/buildSeasonalHeaders.mjs'),
 ...
 assert.match(builder,/webp\(\{ lossless: true, effort: 6 \}\)/);
