...
import{Bell,CircleHelp,DatabaseBackup,Info,MapPin,Newspaper,Settings,Sprout,Users}from'lucide-react';
...
export default function MoreHub({garden,navigate}){
 ...
 return <main className="screen secondary-screen more-screen"><SecondaryHero icon={Sprout} eyebrow="GARDEN COMPASS" title="More" description="Settings, data, help, and app information." className="more-hero"/><section className="screen-pad secondary-screen-content more-content"><div className="secondary-card-list more-compact-list">
  ...
  <SecondaryCard kind="admin" icon={Settings} title="Settings" description="Garden profile, location, frost dates, and alerts" meta={`${location.replace(/\s+54302$/,'')} - Zone ${zone}`} onClick={()=>navigate('admin-location')}/>
  ...
  <SecondaryCard kind="admin" icon={DatabaseBackup} title="Data Management" description="Backup and restore local garden records" onClick={()=>navigate('admin-backup')}/>
  ...
  <SecondaryCard kind="admin" icon={Newspaper} title="What's New" description="Recent user-facing improvements" onClick={()=>navigate('admin-whatsnew')}/>
