...
import{BookOpen,ChevronRight,ClipboardList,PackageSearch,Seedling,Sprout,Warehouse}from'lucide-react';
 ...
 return <main className="screen garden-center-screen">
  ...
  <section className="garden-center-content screen-pad">
   ...
   <div className="garden-center-grid">
    ...
    <Department icon={Seedling} title="Seed Department" subtitle="Review exact packets, varieties, quantities, photos, and notes" status={`${packets} exact packet${packets===1?'':'s'} · ${seeds} saved seed record${seeds===1?'':'s'}`} onClick={()=>navigate('seed-tools')} tone="green"/>
    ...
    <Department icon={PackageSearch} title="Next Season" subtitle="Plan crops, seed purchases, spaces, supplies, and improvements" status={missing.length?`${missing.length} planned crop${missing.length===1?'':'s'} may need seed`:`${planned.length} crop${planned.length===1?'':'s'} in the draft`} onClick={()=>openNext('supplies')} tone="gold"/>
   </div>
   {planned.length>0&&<button type="button" className="next-season-strip" onClick={()=>openNext('crops')}><PackageSearch/><span><strong>{planned.length} crop{planned.length===1?'':'s'} in next season’s draft</strong><small>{missing.length?`Seed may be needed for ${missing.map(item=>cropCatalog.find(crop=>crop.id===item.cropId)?.name).filter(Boolean).slice(0,3).join(', ')}`:'Saved seed currently covers the crop draft.'}</small></span><ChevronRight/></button>}
  </section>
 </main>;
}
