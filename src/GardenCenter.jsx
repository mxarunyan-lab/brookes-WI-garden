...
import{plantingReadinessForPacket}from'./plantingDesk.js';
...
export default function GardenCenter({garden,navigate,tasks=[],dailyDone=[]}){
 const board=buildTaskBoard({tasks,completions:dailyDone}),due=board.counts.dueToday||0,packetRows=(garden.seedPackets||[]).filter(item=>!item.deletedAt),packets=packetRows.length,packetReadiness=packetRows.filter(item=>Number(item.quantity||0)>0).map(packet=>plantingReadinessForPacket({packet,garden})),readyPackets=packetReadiness.filter(row=>row.statusKey==='ready-now').length,soonPackets=packetReadiness.filter(row=>row.statusKey==='outdoor-approaching').length,trays=(garden.trays||[]).filter(item=>!item.deletedAt).length,problems=(garden.problems||[]).filter(item=>item.status!=='resolved'&&!item.deletedAt).length,planned=(garden.yearPlan?.crops||[]).filter(item=>item.status==='definitely'||item.status==='maybe').length,shopping=activeShoppingCount(garden),trip=(garden.vacationPlans||[]).find(plan=>!plan.deletedAt&&plan.status==='active');
 ...
 return <main className="screen secondary-screen garden-center-screen"><SecondaryHero icon={Sprout} eyebrow="ARCHIE & BROOKE’S GARDEN STORE" title="Runyan Garden Center" description="Planning, seeds, care, records, and the next useful garden move." className="garden-center-hero"/><section className="screen-pad secondary-screen-content garden-center-content"><div className="garden-center-compact-grid">
  ...
  <DepartmentTile icon={Sprout} title="Planting Department" status={readyPackets?`${readyPackets} saved packet${readyPackets===1?'':'s'} ready now`:soonPackets?`${soonPackets} packet${soonPackets===1?'':'s'} coming up`:'Grow Now available'} tone="rust" onClick={()=>navigate('plan-plant')}/>
