...
import{BookOpen,ChevronRight,FlaskConical,QrCode,Sprout,Warehouse}from'lucide-react';
...
const SLOGANS=[
 'Questionable expertise since opening day.',
 'Extremely organized seeds. Moderately organized gardeners.',
 'Open daily, weather and motivation permitting.',
 'Wisconsin-grown plans with occasional dirt under the fingernails.',
 'Two directors. Six black beds. No idea where the trowel went.',
 'Helping the Runyans look like they know what they are doing since 2024.'
];
...
export default function GardenCenter({garden,navigate}){
 ...
 const day=Math.floor(Date.now()/86400000),slogan=SLOGANS[day%SLOGANS.length];
 return <main className="screen garden-center-screen">
  ...
  <section className="garden-center-content screen-pad">
   ...
   <Department icon={Sprout} title="Seed Department" subtitle="Seed inventory, packet photos, planting details, and shopping awareness" status={`${seeds} seed record${seeds===1?'':'s'} • ${packets} photographed packet${packets===1?'':'s'}`} onClick={()=>navigate('seed-tools')}/>
   <Department icon={QrCode} title="Label Counter" subtitle="Create and print QR labels for beds, plants, trays, and hydroponic pods" status="Physical garden labels connected to app records" onClick={()=>navigate('garden')}/>
