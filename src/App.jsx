...
export default function App(){
 ...
 const changeStage=(plant,stage,note)=>{const next=normalizeLifecycleStage(stage),current=normalizeLifecycleStage(plant.stage);if(next===current)return touchRecord(plant,actor);const at=new Date().toISOString();return touchRecord({...plant,stage:next,stageUpdatedAt:at,stageHistory:[...normalizeStageHistory(plant,actor),createStageHistoryEntry({stage:next,enteredAt:at,notes:note,actor})]},actor)};
 ...
 const updatePlantStage=id=>setGarden(current=>({...current,plants:current.plants.map(plant=>plant.id===id?changeStage(plant,nextStage(plant.stage),'Lifecycle stage advanced from the Planting Desk.'):plant)}));
 ...
 const completePlanItem=item=>{if(item.type==='succession')setGarden(current=>({...current,succession:current.succession.map(entry=>entry.id===item.id?{...entry,status:'done'}:entry)}));showToast('Planting action recorded.')};
 ...
 const startHardening=plantId=>setGarden(current=>{const exists=(current.hardeningPlans||[]).some(plan=>plan.plantId===plantId&&!plan.deletedAt&&!plan.complete);return{...current,plants:current.plants.map(plant=>plant.id===plantId?changeStage(plant,'Hardening Off','Hardening-off plan started.'):plant),hardeningPlans:exists?current.hardeningPlans:[{id:newId('hardening'),plantId,plantName:current.plants.find(plant=>plant.id===plantId)?.name,day:1,complete:false,createdAt:new Date().toISOString(),actor},...(current.hardeningPlans||[])]}});
 const advanceHardening=id=>setGarden(current=>({...current,hardeningPlans:current.hardeningPlans.map(plan=>{if(plan.id!==id)return plan;const nextDay=Number(plan.day||1)+1;return{...plan,day:nextDay,complete:nextDay>=7,updatedAt:new Date().toISOString(),actor}})}));
 ...
 return <div className="site-stage"><div className={`app-shell page-${page}`}>
  ...
  {page==='chores'&&<ChoreBoard todayTasks={boardTasks} dailyDone={daily.done} onOpen={openTask} onOpenPlant={openPlant} onOpenSpace={openSpace} onStatus={recordTaskStatus} onUndo={undoDaily} navigate={navigate} garden={activeGarden} onAddReminder={addReminder}/>} 
  {page==='garden'&&<BedWorkspace garden={garden} setModal={setModal} manageSpace={manageSpace} saveSpace={saveSpace}/>} 
  {page==='plan-plant'&&<PlanPlantHub recommendations={recommendations} setModal={setModal} garden={garden} timeline={timeline} tasks={boardTasks} dailyDone={daily.done} weather={weatherState.weather} weatherAlert={weatherAlert} completePlanItem={completePlanItem} updatePlantStage={updatePlantStage} scheduleSuccession={scheduleSuccession} saveYearPlan={saveYearPlan} onPlantingDecision={savePlantingDecision} onOpenTask={openTask} onOpenSpace={openSpace} onTaskStatus={recordTaskStatus} onAddReminder={addReminder} startHardening={startHardening} advanceHardening={advanceHardening}/>} 
