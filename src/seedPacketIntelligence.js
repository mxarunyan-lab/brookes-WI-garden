...
export function extractionReview(draft){
 ...
 for(const [key,meta] of entries){const row={key,value:draft[key],...meta};if(!isTrustedPacketValue(key,draft[key],meta)){check.push({...row,reason:'Value did not pass the packet quality check'});continue}if(meta.confidence!=='Low'||meta.source==='manual'||meta.source==='online')ready.push(row);else check.push(row)}
 const exactApproved=Boolean(draft.packetIntelligence?.onlineLookup?.approved);const essential=exactApproved?['name']:['name','variety','brand'];const missing=essential.filter(key=>draft[key]===''||draft[key]===null||draft[key]===undefined||(Array.isArray(draft[key])&&!draft[key].length)).map(key=>({key}));
 const optionalFields=['packetYear','quantity','lotNumber','daysToMaturity'];const optionalMissing=optionalFields.filter(key=>draft[key]===''||draft[key]===null||draft[key]===undefined||(Array.isArray(draft[key])&&!draft[key].length)).map(key=>({key}));return{ready,check,missing,optionalMissing,completedCount:ready.length,attentionCount:check.length+missing.length};
}
