...
function detectGrowingDetails(text,side){
 ...
 const depth=found(text,/(?:plant|sow)(?:\s+seed)?\s*(?:at|about|depth|:)??\s*((?:\d+\s+)?\d+\/\d+|\d+(?:\.\d+)?)\s*(inches?|in\.?|cm)\b/i);if(depth)fields.depth=field(depth,{score:.84,sourcePhoto:side,originalText:lineContaining(lines,/plant|sow/i)});
 const thin=found(text,/(?:thin(?:ning)?\s+(?:to|spacing)?|thin plants to)\s*[:.]?\s*((?:\d+\s+)?\d+\/\d+|\d+(?:\.\d+)?)\s*(inches?|in\.?|cm)\b/i);if(thin)fields.thinningSpacing=field(thin,{score:.82,sourcePhoto:side,originalText:lineContaining(lines,/thin/i)});
 const row=found(text,/(?:row\s+spacing|rows?\s+apart)\s*[:.]?\s*((?:\d+\s+)?\d+\/\d+|\d+(?:\.\d+)?)\s*(inches?|in\.?|cm|feet|ft\.?)\b/i);if(row)fields.rowSpacing=field(row,{score:.82,sourcePhoto:side,originalText:lineContaining(lines,/row/i)});
 const spacing=found(text,/(?:plant\s+spacing|spacing|space\s+plants?)\s*[:.]?\s*((?:\d+\s+)?\d+\/\d+|\d+(?:\.\d+)?)\s*(inches?|in\.?|cm|feet|ft\.?)\b/i);if(spacing)fields.spacing=field(spacing,{score:.8,sourcePhoto:side,originalText:lineContaining(lines,/spacing|space plants/i)});
