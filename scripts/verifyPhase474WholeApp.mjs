...
const seedGarden={
 ...
 spaces:[
  {id:'space-raised',name:'Certification Raised Bed',type:'black-square-bed',capacity:12,hidden:false,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null},
  {id:'space-container',name:'Certification Container Planter',type:'container',capacity:4,hidden:false,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null},
  {id:'space-potato',name:'Certification Potato Grow Bag',type:'potato-grow-bag',capacity:4,hidden:false,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null},
  {id:'space-greenhouse',name:'Certification Greenhouse',type:'greenhouse',capacity:8,hidden:false,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null},
  {id:'space-indoor',name:'Certification Indoor Shelf',type:'indoor',capacity:12,hidden:false,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}
 ],
 plants:[{id:'plant-tomato',operationId:'cert-plant',name:'Certification Tomato With A Deliberately Long Variety Name',cropId:'tomato',cropName:'Tomato',variety:'Long-Name Slicing Tomato (QA)',spaceId:'space-raised',quantity:2,stage:'Growing',status:'active',archived:false,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],
 seedPackets:[{id:'packet-lettuce',operationId:'cert-packet',brand:'Burpee',name:'Lettuce',crop:'Lettuce',cropId:'lettuce',variety:'Iceberg A Long Certification Variety',quantity:100,originalQuantity:100,reservedQuantity:0,status:'active',frontPhoto:'',backPhoto:'',createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],
 reminders:[{id:'reminder-cert',taskId:'reminder-cert',operationId:'cert-reminder',title:'Certification moisture check with a deliberately long task title',taskType:'Check Moisture',priority:95,priorityLabel:'Urgent',dueDate:today,plantId:'plant-tomato',spaceId:'space-raised',reason:'Confirm the soil check workflow remains connected.',note:'Controlled Phase 4.7.4 test record.',manual:true,enabled:true,status:'active',createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],
 trays:[{id:'tray-cert',name:'Certification Tray',crop:'Lettuce',variety:'Iceberg A',cells:12,stage:'Germinating',spaceId:'space-indoor',sourcePacketId:'packet-lettuce',createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],
 growLights:[{id:'light-cert',name:'Certification Basement Lights',onTime:'06:00',offTime:'22:00',checkedToday:false,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],
 hardeningPlans:[],hydroPods:[{id:'pod-cert',position:'1',crop:'Basil',status:'active',createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],greenhouseReadings:[],
 shoppingItems:[{id:'shopping-cert',name:'Certification Compost',quantity:1,purchased:false,status:'active',createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],
 problems:[{id:'problem-cert',plantId:'plant-tomato',spaceId:'space-raised',title:'Certification leaf check',status:'open',createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,deletedAt:null}],harvests:[],activity:[{id:'activity-cert',type:'note',title:'Certification data loaded',detail:'Controlled browser-audit records.',at:now,createdAt:now,updatedAt:now,createdBy:'Archie',updatedBy:'Archie',revision:1,actor:'Archie'}],
 seedUsage:[],taskHistory:[],plantingDecisions:[],yearPlan:{crops:[]},weatherRecommendationHistory:[],vacationPlans:[],calculatorResults:[],seedTransactions:[],wateringEvents:[],soilCheckEvents:[],photos:[],attachments:[],qrLabels:[],offlineOperations:[],succession:[],environmentalRecords:[],environmentalCorrections:[]
};
...
const routes=[
 ...
 route('admin-whatsnew',"Meaningful user-facing changes",page=>moreCard(page,"What's New")),
...
const queryRoutes=[
 ...
 {id:'missing-record',url:'?gardenLabel=plant:missing-certification-record',expected:'cannot be opened'}
];
...
async function auditScreen(page,width,routeId,expected){await page.locator('main').first().waitFor({state:'visible',timeout:10000});const text=(await page.locator('main').first().innerText()).trim(),visibleText=(await page.locator('body').innerText()).trim();assert.ok(text.length>20,`${routeId} appears blank at ${width}px`);assert.match(visibleText,new RegExp(escape(expected),'i'),`${routeId} did not show ${expected} at ${width}px`);const dims=await page.evaluate(()=>({html:[document.documentElement.scrollWidth,document.documentElement.clientWidth],body:[document.body.scrollWidth,document.body.clientWidth],shell:(()=>{const n=document.querySelector('.app-shell');return n?[n.scrollWidth,n.clientWidth]:null})()}));if(dims.html[0]>dims.html[1]+1||dims.shell&&dims.shell[0]>dims.shell[1]+1)findings.push({id:'HORIZONTAL-OVERFLOW',severity:'P1',width,routeId,dims});const actions=await auditVisibleActions(page,width,routeId);return{textLength:text.length,actions,dims}}
async function runRoute(page,width,item,kind,url=base){try{await page.goto(url,{waitUntil:'networkidle',timeout:20000});if(item.open)await item.open(page);const audit=await auditScreen(page,width,item.id,item.expected);results.push({width,routeId:item.id,kind,status:'pass',...audit});if(width===390)await page.screenshot({path:`phase474-audit/whole-app/390-${item.id}.png`,fullPage:true})}catch(error){findings.push({id:'ROUTE-FAILURE',severity:'P1',width,routeId:item.id,kind,detail:String(error?.stack||error)});results.push({width,routeId:item.id,kind,status:'fail',error:String(error?.message||error)});try{if(width===390)await page.screenshot({path:`phase474-audit/whole-app/390-${item.id}-FAILED.png`,fullPage:true})}catch{}}}
...
try{
 for(const width of widths){
  ...
  for(const item of routes)await runRoute(page,width,item,'normal-navigation');
  for(const item of queryRoutes)await runRoute(page,width,item,'query-route',`${base}/${item.url}`);
 ...
 const releaseFindings=findings.filter(item=>['P0','P1','P2'].includes(item.severity)),summary={ok:releaseFindings.length===0,strict,widths,routesTested:routes.length,queryRoutesTested:queryRoutes.length,screenChecks:results.length,findings:findings.length,releaseFindings:releaseFindings.length,bySeverity:findings.reduce((acc,item)=>(acc[item.severity]=(acc[item.severity]||0)+1,acc),{})};
 await writeFile('phase474-audit/whole-app/summary.json',JSON.stringify(summary,null,2));console.log(JSON.stringify(summary,null,2));
 if(strict)assert.equal(releaseFindings.length,0,`Strict whole-app certification found ${releaseFindings.length} P0/P1/P2 issue(s).`);
 else assert.equal(releaseFindings.filter(item=>item.id!=='NAV-001').length,0,'Initial whole-app audit found a P0/P1/P2 issue other than the documented direct-page finding.');
