...
async function auditScreen(page,width,routeId,expected){await page.locator('main').first().waitFor({state:'visible',timeout:10000});const text=(await page.locator('main').first().innerText()).trim();assert.ok(text.length>20,`${routeId} appears blank at ${width}px`);assert.match(text,new RegExp(escape(expected),'i'),`${routeId} did not show ${expected} at ${width}px`);const dims=await page.evaluate(()=>({html:[document.documentElement.scrollWidth,document.documentElement.clientWidth],body:[document.body.scrollWidth,document.body.clientWidth],shell:(()=>{const n=document.querySelector('.app-shell');return n?[n.scrollWidth,n.clientWidth]:null})()}));if(dims.html[0]>dims.html[1]+1||dims.shell&&dims.shell[0]>dims.shell[1]+1)findings.push({id:'HORIZONTAL-OVERFLOW',severity:'P1',width,routeId,dims});const actions=await auditVisibleActions(page,width,routeId);return{textLength:text.length,actions,dims}}
