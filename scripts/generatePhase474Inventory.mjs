...
for(const file of files){const source=await readFile(file,'utf8'),relative=path.relative(process.cwd(),file).replaceAll('\\','/');
 ...
 let match;while((match=tagRe.exec(source))){const[tag,element,attrs]=match,line=source.slice(0,match.index).split('\n').length,label=attr(attrs,'aria-label')||attr(attrs,'title')||visibleText(source,match.index+tag.length),handler=attr(attrs,'onClick')||attr(attrs,'onSubmit')||attr(attrs,'onChange')||attr(attrs,'href'),className=attr(attrs,'className'),disabled=/\bdisabled\b/.test(attrs),type=attr(attrs,'type')||element;
  const row={actionId:`ACT-${String(actions.length+1).padStart(4,'0')}`,file:relative,line,element,type,label:label||'(no visible label extracted)',className,handler:handler||'',disabled,initialRisk:''};
  if(element==='button'&&!handler)row.initialRisk='button-without-explicit-handler';
  if(element==='a'&&(!handler||handler==='#'))row.initialRisk='link-without-usable-destination';
  if(element==='input'&&!attr(attrs,'aria-label')&&!/type=["']?(hidden|file)/.test(attrs))row.initialRisk='input-label-needs-browser-verification';
...
const headers=['actionId','file','line','element','type','label','className','handler','disabled','initialRisk'];
