...
const replacements=[
 ...
 {
  label:'Phase 4.7.5 context health capability mock',
  old:[
   ...
   "await page.route('**/api/health',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,version:'0.20.4',packetVisionConfigured:true})}));",
  ...
  next:"await context.route('**/api/health',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,version:'0.20.4',packetVisionConfigured:true})}));",
 },
 {
...
for(const{label,old,next}of replacements){
 ...
 const current=old.find(value=>source.includes(value));
 if(!current)throw new Error(`${path}: expected source not found for ${label}`);
