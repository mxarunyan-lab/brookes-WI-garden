...
for(let attempt=1;attempt<=attempts;attempt+=1){try{
 ...
 assert.equal(healthResponse.status,200);const health=await healthResponse.json();assert.equal(health.ok,true);assert.equal(health.version,'0.20.9');
 const html=await fetchText(`${base}/?phase481=${Date.now()}`),scriptPaths=[...html.matchAll(/<script[^>]+src="([^"]+\.js)"/g)].map(m=>m[1]),stylePaths=[...html.matchAll(/<link[^>]+href="([^"]+\.css)"/g)].map(m=>m[1]);
 assert.ok(scriptPaths.length);assert.ok(stylePaths.length);
 ...
 assert.match(css,/garden-header-summer\.webp\?v=0479/);assert.match(css,/aspect-ratio:2(?:\/1)?/);assert.match(css,/background-size:contain/);assert.match(css,/tool-shed-category-summary/);assert.match(css,/more-settings-hub:not\(\[open\]\)>div/);assert.match(css,/var\(--green-950\)/);
...
const liveSeason=currentGreenBaySeason(),seasonalAsset=`/images/garden-headers/garden-header-${liveSeason}.webp?v=0479`,assetResponse=await fetch(`${base}${seasonalAsset}`,{cache:'no-store'});
assert.equal(assetResponse.status,200);assert.match(assetResponse.headers.get('content-type')||'',/image\/webp/);assert.ok((await assetResponse.arrayBuffer()).byteLength>10000);
...
try{const widths=[320,375,390,430];for(const width of widths){
 ...
 await goto('today');const hero=page.locator('.compact-home-hero'),todayCard=page.locator('.what-matters-today');await hero.waitFor();await todayCard.waitFor();
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth),true);const heroBox=await hero.boundingBox(),cardBox=await todayCard.boundingBox();assert.ok(Math.abs(heroBox.width/heroBox.height-2)<.04);assert.ok(cardBox.y<heroBox.y+heroBox.height);
 const heroStyle=await hero.evaluate(el=>({backgroundImage:getComputedStyle(el).backgroundImage,backgroundSize:getComputedStyle(el).backgroundSize,borderRadius:getComputedStyle(el).borderRadius}));assert.ok(heroStyle.backgroundImage.includes(`garden-header-${liveSeason}.webp`));assert.equal(heroStyle.backgroundSize,'contain');assert.match(heroStyle.borderRadius,/22px 22px 0px 0px|18px 18px 0px 0px/);
 assert.equal(await page.locator('.watering-can-icon').count(),1);
 ...
 await goto('tools');assert.equal(await page.locator('.tool-shed-category').count(),3);assert.equal(await page.locator('.tool-shed-card-list').count(),0);await page.getByRole('button',{name:/Weather & Timing/}).click();assert.equal(await page.getByText('Garden Weather & Timing',{exact:true}).count(),1);
 await page.getByText('Garden Weather & Timing',{exact:true}).click();await page.getByRole('heading',{name:'Garden Weather & Timing'}).waitFor();assert.equal(await page.getByRole('tab').count(),3);
 await context.close();}
